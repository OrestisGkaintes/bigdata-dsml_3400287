...
Ο οδηγός αυτός βασίζεται στους τοπικούς οδηγούς `docs/02_vscode-local-authoring`, `docs/02_pycharm-local-authoring` και `docs/03_local-spark-workbook` και μεταφέρει τα ίδια ερωτήματα στο απομακρυσμένο περιβάλλον εκτέλεσης.

Η εκτέλεση γίνεται μόνο από WSL, πάνω στο περιβάλλον που ρυθμίστηκε στον οδηγό `04_remote-spark-kubernetes`.

Η βασική αρχή είναι:

- το ερώτημα μένει ίδιο
- το βασικό script μένει ίδιο
- αλλάζει μόνο ο τρόπος εκτέλεσης και οι διαδρομές των δεδομένων

Με αυτόν τον τρόπο ο φοιτητής δεν καλείται να μάθει ταυτόχρονα νέο API και νέα υποδομή.

## Τι θα μάθετε

Στο τέλος αυτού του εργαστηρίου θα μπορείτε να:

- τρέχετε Spark jobs από το δικό σας WSL μέσω VPN
- διαβάζετε δεδομένα από το HDFS με `--base-path`
- συγκρίνετε RDD, DataFrame API και Spark SQL πάνω στα ίδια datasets
- ξεχωρίζετε τα βασικά ερωτήματα από τα επιπλέον παραδείγματα και το παράρτημα

## Πριν ξεκινήσεις

Συνιστώμενη σειρά:

1. [docs/02_vscode-local-authoring/README.md](../02_vscode-local-authoring/README.md)
2. εναλλακτικά [docs/02_pycharm-local-authoring/README.md](../02_pycharm-local-authoring/README.md)
3. [docs/03_local-spark-workbook/README.md](../03_local-spark-workbook/README.md)
4. [docs/04_remote-spark-kubernetes/README.md](../04_remote-spark-kubernetes/README.md) για τη ροή εκτέλεσης στο Kubernetes

Ο παρών οδηγός υποθέτει ότι:

- έχεις ολοκληρώσει το WSL/VPN setup του [οδηγού απομακρυσμένης εκτέλεσης σε Kubernetes](../04_remote-spark-kubernetes/README.md)
- `spark-submit`, `kubectl` και `hdfs` βρίσκονται ήδη στο `PATH`
- υπάρχει per-user `spark-defaults.conf` που στοχεύει τη συστοιχία του εργαστηρίου
- το HDFS namespace του χρήστη σου είναι διαθέσιμο

Αν μόλις έρχεστε από τον τοπικό οδηγό εξάσκησης, βγείτε πρώτα από το `.venv` και ξαναφορτώστε το περιβάλλον Spark του WSL:
...
## Τα δεδομένα και τα βασικά scripts

Τα βασικά αρχεία δεδομένων είναι:
...
| Ερώτημα | RDD | DataFrame API | Spark SQL |
| --- | --- | --- | --- |
| Q1: 5 χαμηλότεροι μισθοί | `code/RddQ1.py` | `code/DFQ1.py` | `code/SQLQ1.py` |
| Q2: 3 πιο καλοπληρωμένοι του Dep A | `code/RddQ2.py` | `code/DFQ2.py` | `code/SQLQ2.py` |
| Q3: ετήσιο εισόδημα | `code/RddQ3.py` | `code/DFQ3.py` | `code/SQLQ3.py` |
...
```bash
spark-submit hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER/code/<script>.py \
...
- με το per-user `spark-defaults.conf` του [οδηγού απομακρυσμένης εκτέλεσης σε Kubernetes](../04_remote-spark-kubernetes/README.md), το plain `spark-submit` στο WSL τρέχει ήδη σε λειτουργία συστοιχίας Kubernetes
- αν θέλεις να παρακάμψεις προσωρινά αυτή τη ρύθμιση, μπορείς να δώσεις ρητά άλλο `--master` ή άλλο `--deploy-mode`

## Εισαγωγικό παράδειγμα: Word Count

```bash
spark-submit hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER/code/wordcount.py \
  --base-path hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER
```
...
## Q1: 5 χαμηλότεροι μισθοί

Το ερώτημα είναι το ίδιο σε όλα τα APIs:

- διάβασε το `employees.csv`
- ταξινόμησε κατά αύξοντα μισθό
- κράτησε μόνο τις 5 πρώτες γραμμές
...
```bash
spark-submit hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER/code/DFQ1.py \
  --base-path hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER
```
...
```bash
spark-submit hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER/code/SQLQ1.py \
  --base-path hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER
```
...
## Q2: 3 πιο καλοπληρωμένοι του Dep A

Το ερώτημα είναι:

- κάνε join εργαζομένων και τμημάτων
- κράτησε μόνο το `Dep A`
- ταξινόμησε κατά φθίνοντα μισθό
- επέστρεψε μόνο 3 γραμμές

### Q2 με RDD

```bash
spark-submit hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER/code/RddQ2.py \
  --base-path hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER
```
...
```bash
spark-submit hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER/code/DFQ2.py \
  --base-path hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER
```
...
```bash
spark-submit hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER/code/SQLQ2.py \
  --base-path hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER
```
...
- υπολόγισε `yearly_income = salary * 14`
- εμφάνισε τα αποτελέσματα ταξινομημένα κατά φθίνον ετήσιο εισόδημα

### Q3 με RDD

```bash
spark-submit hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER/code/RddQ3.py \
  --base-path hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER
```
...
```bash
spark-submit hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER/code/DFQ3.py \
  --base-path hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER
```
...
```bash
spark-submit hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER/code/SQLQ3.py \
  --base-path hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER
```
...
### Άθροισμα μισθών ανά τμήμα με DataFrame API

Αυτό είναι επιπλέον tabular example και δεν ανήκει στον βασικό κορμό `Q1-Q3`.

```bash
spark-submit hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER/code/DF2b.py \
  --base-path hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER
```
...
### Το ίδιο Q3 με DataFrame UDF

Το `DFQ3_udf.py` κρατιέται ως επιπλέον παράδειγμα και όχι ως βασική DataFrame υλοποίηση του `Q3`.

```bash
spark-submit hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER/code/DFQ3_udf.py \
  --base-path hdfs://hdfs-namenode.default.svc.cluster.local:9000/user/$DSML_USER
```
...
## Παράρτημα: Απλοποιημένο παράδειγμα join με δεδομένα στη μνήμη

Το `RddQ4.py` δεν χρησιμοποιεί τα ίδια CSVs με τον βασικό κορμό του εργαστηρίου. Είναι ένα απλοποιημένο παράδειγμα με δεδομένα στη μνήμη, ώστε να φανεί πώς μπορεί να στηθεί join λογική μόνο με RDD transformations.
...
## Τι να κρατήσετε από αυτό το εργαστήριο

- τα `Q1-Q3` έχουν πλέον καθαρό matrix `RDD / DataFrame / SQL`
- το όνομα των βασικών scripts δηλώνει πλέον και το API που βλέπει ο φοιτητής
- το `DF2b.py` και το `DFQ3_udf.py` είναι extras, όχι core συγκρίσεις
- το `RddQ4.py` είναι παράρτημα και όχι canonical `Q4`
